#!/usr/bin/env python3
"""자막 수집 배치 — vm4 movies 순회, vm5 status 멱등 원장 (모두 REST).

env: SUBDL_API_KEY, AI_DATABASE_URL, AI_DATABASE_KEY, DATA_SUPABASE_URL, DATA_SUPABASE_KEY
"""
import os
import time
...
def process_movie(http, tmdb_id: int) -> str:
    """영화 1편 처리. 반환: 'done'|'skipped'|'failed'|'cached'.
    SubdlRateLimit는 호출자가 처리하도록 전파."""
    if db.get_state(tmdb_id) == "done":
        return "cached"
    try:
        chosen = sel.choose(subdl.search(tmdb_id, http))
        if chosen is None:
            db.set_status(tmdb_id, "skipped")
            return "skipped"
        raw_text = subdl.download_and_extract(chosen.get("url") or "", http)
        if not raw_text.strip():
            db.set_status(tmdb_id, "failed", "empty srt")
            return "failed"
        db.save_subtitle(tmdb_id, chosen, raw_text)
        db.set_status(tmdb_id, "done")
        return "done"
    except subdl.SubdlRateLimit:
        raise
    except Exception as e:  # noqa: BLE001 — 어떤 오류든 failed로 기록하고 계속
        db.set_status(tmdb_id, "failed", str(e)[:500])
        return "failed"


def run() -> None:
    if not os.getenv("AI_DATABASE_URL"):
        raise SystemExit("AI_DATABASE_URL 환경변수가 필요합니다 (vm5 REST).")

    counts = {"done": 0, "skipped": 0, "failed": 0, "cached": 0}
    requests_made = 0

    with httpx.Client(timeout=60, verify=False) as http:
        for tmdb_id in db.iter_movies():
            ...
            if requests_made >= MAX_REQUESTS_PER_RUN:
                ...
                break
            try:
                result = process_movie(http, tmdb_id)
            except subdl.SubdlRateLimit:
