...
|---|---|---|---|
| 4K Cinema FE | `4K_FE/**` | `ghcr.io/sanggyoon/4k-fe` | `Ansible/manifests/4k-fe/` |
...
GitHub Actions Runner (ubuntu-latest)
  ...
  │
  ├─ ② GHCR 푸시
  │     ghcr.io/sanggyoon/4k-fe:<sha>   ← 불변 태그
  │     ghcr.io/sanggyoon/4k-fe:latest  ← 가변 태그
  │
  ├─ ③ kustomization.yaml 태그 업데이트
  │     Ansible/manifests/4k-fe/kustomization.yaml
...
```
git commit -m "ci: update 4k-fe image → abc1234 [skip ci]"
```
...
FE (`argocd-app-4k-fe.yaml`):
```yaml
spec:
  source:
    repoURL: https://github.com/sanggyoon/202605_KakaoCloud_AIaaS.git
    targetRevision: main
    path: Ansible/manifests/4k-fe   # Kustomize 자동 인식
  destination:
...
> BE Application 등록 완료. `kubectl get applications -n argocd` → `4k-fe`, `4k-be`, `infra-manifests` 3개 모두 `Synced/Healthy`.

### 이미지 태그 업데이트 경로

```
GitHub Actions
  → Ansible/manifests/4k-fe/kustomization.yaml 수정 (newTag 변경)
  → git push
  → ArgoCD 감지 (~3분)
...
GitHub → Profile → Packages → `4k-fe` → Package settings → **Change visibility → Public**
...
| `Ansible/manifests/4k-fe/*.yaml` 직접 수정 후 push | ArgoCD가 감지 → 클러스터 적용 (CI 미실행) |
...
| ArgoCD OutOfSync 해소 안 됨 | `Ansible/manifests/4k-fe/` 경로·repoURL 오타, Git 인증 확인 |
...
| 항목 | 방법 |
|------|------|
| Supabase ArgoCD 편입 | 현재 helm 직접 배포 → `argocd-app-supabase-*.yaml` 등록해 GitOps self-heal 적용 (장애 시 수동 helm upgrade 제거) |
