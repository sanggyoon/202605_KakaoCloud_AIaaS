...
**날짜:** 2026-06-09
**상태:** 승인됨
**상위 맥락:** 4K Cinema 피크-스코어 ML 파이프라인 step 2. 하위 프로젝트 A(vm5 `public` 스키마)를 토대로, subdl에서 영화 자막을 수집해 vm5 `public.subtitles`에 적재한다. 이후 C(파싱)가 이 데이터를 소비한다.
...
- subdl에서 영화별 영어 자막 1개를 규칙 선택·다운로드해 `public.subtitles`에 적재.
...
- `db.py` — DB 입출력 (모두 Supabase REST/PostgREST. 테이블은 public 스키마라 별도 프로파일 헤더 불필요. 직접 Postgres(psycopg) 아님 — vm5 5432 포트 비공개.)
...
`db/schema.sql`은 public 스키마 기준으로 정리됨(`subtitles.is_sdh boolean` 포함). vm5는 비어 있어 기존 `training` 스키마를 드롭하고 public으로 재생성:

```sql
drop schema if exists training cascade;   -- 이전 빈 테이블 정리
-- 이후 schema.sql 전체를 SQL Editor에 붙여넣어 public에 생성
```
...
`SUBDL_API_KEY`(신규), `AI_DATABASE_URL`·`AI_DATABASE_KEY`(vm5 REST, 비공개 schema=public), `DATA_SUPABASE_URL`·`DATA_SUPABASE_KEY`(vm4 movies 읽기, 기존), 선택 `AI_BASIC_USER`/`AI_BASIC_PASS`·`DATA_BASIC_USER`/`DATA_BASIC_PASS`(nginx basic auth). 로컬은 `4K_ML/.env`.
