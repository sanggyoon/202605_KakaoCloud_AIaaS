...
- **proxy 신규** — `4K_FE/proxy.ts` (Next 16에서 `middleware`는 deprecated,
  `proxy.ts` + `export function proxy`로 대체. Proxy는 기본적으로 Node.js 런타임에서
  실행되므로 `node:crypto`를 쓰는 `isValidSession`이 그대로 동작):
  - `config.matcher`로 `/manager`, `/movie_list`(및 하위 `/movie_list/...`) 보호.
